...
package im.heart.continew.member.auth.enums;
...
public enum MemberPasswordPolicyEnum {
    ...
    PASSWORD_MIN_LENGTH("密码最小长度取值范围为 %d-%d", 8, 32, "密码最小长度为 %d 个字符") {
        @Override
        public void validate(String password, int value, MemberDO user) {
            // 最小长度校验
    ...
    PASSWORD_REPETITION_TIMES("历史密码重复校验次数取值范围为 %d-%d", 3, 32, "新密码不得与历史前 %d 次密码重复") {
        @Override
        public void validate(String password, int value, MemberDO user) {
            MemberPasswordHistoryService userPasswordHistoryService = SpringUtil
                .getBean(MemberPasswordHistoryService.class);
