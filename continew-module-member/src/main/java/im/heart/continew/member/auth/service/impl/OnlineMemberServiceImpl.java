...
import im.heart.continew.member.auth.model.query.OnlineMemberQuery;
import im.heart.continew.member.auth.model.resp.OnlineMemberResp;
...
public class OnlineMemberServiceImpl implements OnlineMemberService {
    ...
    @Override
    @AutoOperate(type = OnlineMemberResp.class, on = "list")
    public PageResp<OnlineMemberResp> page(OnlineMemberQuery query, PageQuery pageQuery) {
        List<OnlineMemberResp> list = this.list(query);
        return PageResp.build(pageQuery.getPage(), pageQuery.getSize(), list);
    }

    @Override
    public List<OnlineMemberResp> list(OnlineMemberQuery query) {
        List<OnlineMemberResp> list = new ArrayList<>();
        // 查询所有在线 Token
        ...
        for (Map.Entry<Long, List<String>> entry : tokenMap.entrySet()) {
            ...
            entry.getValue().parallelStream().forEach(token -> {
                ...
                OnlineMemberResp resp = BeanUtil.copyProperties(userContext, OnlineMemberResp.class);
        ...
        // 设置排序
        CollUtil.sort(list, Comparator.comparing(OnlineMemberResp::getLoginTime).reversed());
        return list;
    }
