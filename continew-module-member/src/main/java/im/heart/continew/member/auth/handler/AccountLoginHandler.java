...
import im.heart.continew.member.auth.model.req.AccountLoginReq;
...
public class AccountLoginHandler extends AbstractMemberLoginHandler<AccountLoginReq> {

    private final PasswordEncoder passwordEncoder;

    @Override
    public MemberLoginResp login(AccountLoginReq req, ClientResp client, HttpServletRequest request) {
        // 解密密码
        String rawPassword = ExceptionUtils.exToNull(() -> SecureUtils.decryptByRsaPrivateKey(req.getPassword()));
        ValidationUtils.throwIfBlank(rawPassword, "密码解密失败");
        // 验证用户名密码
        String username = req.getUsername();
        MemberDO member = memberService.getByAccount(username);
        boolean isError = ObjectUtil.isNull(member) || !passwordEncoder.matches(rawPassword, member.getPassword());
        // 检查账号锁定状态
        ...
        // 执行认证
        String token = this.authenticate(member, client);
