...
import im.heart.continew.member.auth.model.req.PhoneLoginReq;
 ...
 */
@Component
public class PhoneLoginHandler extends AbstractMemberLoginHandler<PhoneLoginReq> {

    @Override
    public MemberLoginResp login(PhoneLoginReq req, ClientResp client, HttpServletRequest request) {
        // 验证手机号
        MemberDO member = memberService.getByPhone(req.getPhone());
        ValidationUtils.throwIfNull(member, "此手机号未绑定本系统账号");
        // 检查用户状态
